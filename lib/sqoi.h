...
#define QOI_OP_RUN 0xC0
  ...
  *byte_ptr = ((value >> 24) & 0xFF | (value << 8) & 0xFF0000 | (value >> 8) & 0xFF00 | (value >> 24) & 0xFF000000);
  ...
  static bool qoi_cmp_pixel(qoi_pixel_t pixel1, qoi_pixel_t pixel2, const uint8_t channels)
  {
    bool is_same_pixel = (pixel1.colors.red == pixel2.colors.red &&
                          pixel1.colors.green == pixel2.colors.green &&
                          pixel1.colors.blue == pixel2.colors.blue);

    if (channels > 3)
      is_same_pixel = is_same_pixel && (pixel1.colors.alpha == pixel2.colors.alpha);
  ...
  {
    pixel->colors.red = red;
    pixel->colors.green = green;
    pixel->colors.blue = blue;
  }
  ...
  static inline int32_t qoi_get_index_position(qoi_pixel_t pixel)
  {
    return (pixel.colors.red * 3 + pixel.colors.green * 5 + pixel.colors.blue * 7 + pixel.colors.alpha * 11) % 64;
  }
    ...
    */
    ...
    else
    {
      ...
      else
      {
        ...
        if (desc->channels > 3 && cur_pixel.colors.alpha != enc->prev_pixel.colors.alpha)
        {
          uint8_t tag[5] =
              {
                  QOI_OP_RGBA,
                  cur_pixel.colors.red,
                  cur_pixel.colors.green,
                  cur_pixel.colors.blue,
                  cur_pixel.colors.alpha};
        ...
        else
        {
          ...
          red_dif = cur_pixel.colors.red - enc->prev_pixel.colors.red;
          green_dif = cur_pixel.colors.green - enc->prev_pixel.colors.green;
          blue_dif = cur_pixel.colors.blue - enc->prev_pixel.colors.blue;
  ...
  qoi_pixel_t qoi_decode_chunk(qoi_dec_t *dec)
  {
    ...
    else
    {
      ...
      */
      ...
      {
        dec->prev_pixel.colors.red = dec->offset[1];
        dec->prev_pixel.colors.green = dec->offset[2];
        dec->prev_pixel.colors.blue = dec->offset[3];
        dec->offset += 4;
      }
      else if (tag == QOI_OP_RGBA) /* RGBA pixel */
      {
        ...
        dec->prev_pixel.colors.green = dec->offset[2];
        dec->prev_pixel.colors.blue = dec->offset[3];
        dec->prev_pixel.colors.alpha = dec->offset[4];
        dec->offset += 5;
      }
      else
      {
        ...
        case QOI_OP_DIFF:
        {
          ...
          dec->prev_pixel.colors.red += red_diff;
          dec->prev_pixel.colors.green += green_diff;
          dec->prev_pixel.colors.blue += blue_diff;
          dec->offset += 1;
        ...
        case QOI_OP_LUMA:
        {
          ...
          dec->prev_pixel.colors.red += lumaGreen + ((dec->offset[1] & 0xF0) >> 4) - 8;
          dec->prev_pixel.colors.green += lumaGreen;
          dec->prev_pixel.colors.blue += lumaGreen + (dec->offset[1] & 0x0F) - 8;
